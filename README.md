...
ssh root@your-server "cd /opt/weather-edge && sudo -u weather git pull && systemctl restart weather-edge"
```
...
Open-Meteo (6-8 models/city)
  ...
  Edge Detection + Kelly Sizing  <---  Market Prices + Order Book Asks
        ...
        +-- Penny pool (10% bankroll, TopTrader-style sniping)
...
- **Penny pool (10%)**, TopTrader-style penny sniping. Buy YES at $0.01-$0.06 where model says 3x+ higher probability. Most lose, but 50:1 payoffs compound.

## Spread Capture (TopTrader Strategy)
...
- **Competitor tracking**, Monitors @TopTrader's public stats + on-chain wallet activity
