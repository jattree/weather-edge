...
Inspired by [@TopTrader's Claude Trader](https://polymarket.com/profile/@TopTrader) ($77K+ positions, 4,300+ predictions, 73% win rate, ~$37K monthly P&L).
...
8. Runs dual strategy: 70% core bets + 30% TopTrader-style penny tail bets
...
The system runs on **your-server** (Rocky Linux 9.7):
...
- **Deploy**: `/opt/weather-edge/deploy.sh` (git pull + pip install + restart)
...
ssh root@your-server "su - weather -c '/opt/weather-edge/deploy.sh'"
```
...
| Component | Detail |
|-----------|--------|
| Host | your-server (your-server) |
| OS | Rocky Linux 9.7 (Blue Onyx) |
| Python | 3.11 |
| Timezone | UTC |
| App user | `weather` (restricted) |
| Repo | `/opt/weather-edge` |
| Branch | `feature/ui-redesign` |
| DB | `/opt/weather-edge/weather_edge.db` (SQLite) |
| Config | `/opt/weather-edge/.env` |
...
```bash
git clone git@git.example.com:trading/weather-edge.git
cd weather-edge
...
- **URL**: https://git.example.com:9443/trading/weather-edge
- **Group**: trading
...
- **Tail bets (30% of bankroll)**, TopTrader-style penny sniping. Buy YES on buckets priced at $0.01-$0.05 where our model says 3x+ higher probability. Most lose, but 50:1 payoffs on the winners compound.
...
- **@TopTrader**, $77K+ positions, 4,300+ predictions, 73% win rate, ~$37K monthly P&L
...
- Displayed in dashboard sidebar: "WeatherEdge +$X vs toptrader +$Y"
...
- **Competitor tracking**, Monitors @TopTrader's public stats for performance comparison
- **Quarter-Kelly sizing**, Conservative position sizing (TopTrader-validated)
...
src/weather_edge/
  ...
  analysis/
    ...
    competitor_tracker.py # @TopTrader and whale tracking
