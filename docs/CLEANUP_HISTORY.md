...
| Hygiene | The quickstart `run` command crashed. Undeclared dependencies, a dead Postgres ORM layer, and undocumented settings were fixed; `run_cycle` (cyclomatic complexity 193) was split into stages, and a ruff plus complexity ratchet guards against regressions (see CONTRIBUTING.md) |
